...
- Sprint 17 bugs: #1 stack UI, #2 over-merge, #3 sticky alignment, #4 shift+click conflict
...
```typescript
designMode: 'smart' | 'detail'   // default: 'smart'
toggleDesignMode: () => void
setDesignMode: (mode: 'smart' | 'detail') => void
```

**Behavior:**
- `'smart'` (default): All auto-consequence cascades fire normally (auto-railing, entry wall clearing, merge, etc.)
- `'detail'`: All auto-consequence functions become no-ops. User paints faces exactly as specified. Existing state is preserved (no retroactive changes).
- Switching `detail → smart`: Runs a validation pass via the Phase 5 engine and surfaces warnings for anything smart mode *would have* auto-fixed. Does NOT auto-fix — just warns. User decides.

**Guard placement:** Every auto-consequence call site (`recomputeSmartRailings`, stair entry wall clearing, `mergeBoundaryVoxels`) gets a `if (designMode === 'detail') return` guard at the top. Access via `useStore.getState().designMode` inside store actions (no selector needed — read at call time).
...
### 1c. Sprint 17 Bug #1: Stack UI

Add a "Stack Container" option to `ContainerContextMenu.tsx`:
- Only shown when a suitable base container exists below (position overlap check)
- Calls existing `stackContainer(selectedId, targetId)`
- `targetId` determined by position overlap using existing `findContainerBelow` helper
- If ambiguous (multiple containers below), show sub-menu with options
- Toast confirmation via sonner: "Stacked Container 2 on Container 1"

### 1d. Sprint 17 Bug #2: Partial-Overlap Merge Fix (Phase 3)

Fix `mergeBoundaryVoxels` in `containerSlice.ts`:
- Current: merges ALL boundary voxels when containers are adjacent
- Fixed: compute per-voxel world-space AABB overlap between adjacent containers
- Only voxels whose world-space extents actually overlap get merged
- Uses existing `CONTAINER_DIMENSIONS` + voxel pitch math to compute per-voxel world bounds
- `adjacencyDetection.ts` already has AABB math — extend with per-voxel granularity

### 1e. Sprint 17 Bug #3: Sticky Alignment in addContainer

Wire snap logic into the `addContainer` smart placement path:
- Current: `STICKY_THRESHOLD=0.3` only applies during `moveContainer` drag
- Fixed: after `addContainer` computes initial position, run the same snap check against existing containers
- If within threshold, snap to flush alignment before finalizing position

### 1f. Sprint 17 Bug #4: Shift+Click Camera Conflict

Disable `CameraControls` when Shift is held:
- Add `shiftHeld: boolean` to `uiSlice` (ephemeral)
- Set via keyboard event listeners in `useAppHotkeys` (keydown/keyup for Shift)
- `CameraControls` gets `enabled={!shiftHeld && !isDragging}`
- This prevents camera orbit from fighting container drag when Shift is held
...
| Rule | Trigger | Severity |
|------|---------|----------|
| `checkGravity` | Body voxel at level > 0 with no active voxel directly below in the container below (using `findContainerBelow`) | `warning` |
...
Rather than instrumenting every store action, use a Zustand `subscribe` with shallow equality on the containers slice:
...
useEffect(() => {
  ...
  const unsub = useStore.subscribe(
    ...
    (containers) => {
      clearTimeout(timer);
      timer = setTimeout(() => {
        const warnings = validateDesign(containers, budgetThreshold);
...
- Use the stair ascending direction (`_smartStairChanges` or `stairDirection` field) to orient steps
...
```
Stream 1: Store & Data Model ──────────────────────────┐
  ├─ 1a. Detail mode toggle (uiSlice)                  │
  ├─ 1b. Warning state + DesignWarning type             │
  ├─ 1c. Bug #1: Stack UI context menu action           │
  ├─ 1d. Bug #2: Partial-overlap merge fix              │
  ├─ 1e. Bug #3: Sticky alignment in addContainer       │
  └─ 1f. Bug #4: shiftHeld flag + CameraControls guard  │
                                                        │
...
Stream 3: 2D UI ────────────────────────────────────────┤
  ...
  ├─ 3c. Detail/Smart toggle (toolbar)                  │
  └─ 3d. Stack button (context menu)                    │
                                                        │
Stream 4: 3D Rendering ────────────────────────────────┘
...
- Within Stream 1: bug fixes 1c–1f are independent of each other
...
| `src/__tests__/detail-mode.test.ts` | Tests for Smart/Detail toggle behavior |
| `src/__tests__/bug-fixes-sprint17.test.ts` | Tests for all 4 bug fixes |

### Modified Files

| File | Changes |
|------|---------|
| `src/store/slices/uiSlice.ts` | `designMode`, `warnings`, `hoveredWarning`, `shiftHeld` state + actions |
| `src/store/slices/containerSlice.ts` | `mergeBoundaryVoxels` partial-overlap fix, sticky alignment in `addContainer` |
| `src/store/slices/voxelSlice.ts` | `designMode` guard on `recomputeSmartRailings`, stair entry wall clearing |
| `src/components/ui/TopToolbar.tsx` | Smart/Detail toggle buttons |
| `src/components/ui/Sidebar.tsx` | Mount WarningPanel section |
| `src/components/ui/ContainerContextMenu.tsx` | "Stack Container" menu item |
...
| `src/components/three/WalkthroughControls.tsx` | Stair collision ramp generation |
| `src/components/three/Scene.tsx` | Mount WarningOverlay, ValidationSubscriber |
| `src/hooks/useAppHotkeys.ts` | Shift key tracking (keydown/keyup) |
...
| `shiftHeld` | No (ephemeral) | No | Keyboard state |
...
- Auto-fix on detail→smart switch (warns only, does not auto-modify)
