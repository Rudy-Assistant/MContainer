...
1. **Always-visible level chip strip** — `BlueprintLevelChips.tsx` renders `All(N) | L2 | L1 | Pool` chips inline in the TopToolbar (ZONE B, next to the view-mode tabs). Click any chip to filter the rendered level. Originally rendered as a separate top-center overlay alongside the right-side LevelSlicer dropdown; consolidated into a single topbar element per Bruce 2026-05-06 round-3 audit. The strip is the only level selector and is shown in 3D and Blueprint modes (hidden in walkthrough). LevelSlicer.tsx was removed.
