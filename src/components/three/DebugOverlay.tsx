 ...
 *
 * Renders bay/voxel boundary wireframes using EdgesGeometry.
...
function ContainerDebugWireframe({ container }: { container: Container }) {
  ...
  // Bay group wireframes (Simple mode) — all 15 groups
  ...
  // Per-voxel wireframes (Detail mode) — all 32 voxels
  const voxels = useMemo(() => {
    if (isSimpleMode) return null;
    const TOTAL = VOXEL_ROWS * VOXEL_COLS;
    const result: { px: number; pz: number; w: number; d: number; isExt: boolean }[] = [];
    for (let i = 0; i < TOTAL; i++) {
      ...
      result.push({ px: layout.px, pz: layout.pz, w: layout.voxW, d: layout.voxD, isExt: isHaloCol || isHaloRow });
    }
    return result;
